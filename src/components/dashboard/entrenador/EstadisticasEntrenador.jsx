// components/dashboard/entrenador/EstadisticasEntrenador.jsx
import React from 'react'
import { TrendingUp } from 'lucide-react'

const EstadisticasEntrenador = ({ user }) => {
  return (
      ...
      <div className="bg-[#1a1a1a]/80 backdrop-blur-sm border-b border-[#2d2d2d] px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm">Visualiza el progreso de tus alumnos</p>
          </div>
        </div>
      </div>
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          <div className="w-20 h-20 bg-[#e31837]/10 rounded-full flex items-center justify-center mx-auto mb-4">
            <TrendingUp className="w-10 h-10 text-[#e31837]" />
          </div>
          <h2 className="text-xl font-bold text-white mb-2">Módulo en Desarrollo</h2>
          <p className="text-gray-400">Próximamente podrás ver estadísticas detalladas de tus alumnos.</p>
        </div>
      </div>
    </div>
  )
